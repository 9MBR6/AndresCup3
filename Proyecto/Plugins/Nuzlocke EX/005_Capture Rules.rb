...
ItemHandlers::CanUseInBattle.addIf(:poke_balls,
  ...
  proc { |item, pokemon, battler, move, firstAction, battle, scene, showMessages|
    ...
    if !firstAction
      scene.pbDisplay(_INTL("Solo puedes usar una pokeball si no realizas ninguna acción con el otro pokémon")) if showMessages
      next false
    end
    if battler.semiInvulnerable?
      scene.pbDisplay(_INTL("no se pue, vamos que no se pue apuntar si no lo veo")) if showMessages
      next false
    end
    ...
    if battle.pbOpposingBattlerCount > 1 && !(GameData::Item.get(item).is_snag_ball? && battle.trainerBattle?)
      if battle.pbOpposingBattlerCount == 2
        scene.pbDisplay(_INTL("Debes sacrificar a 1 pokémon para capturar al otro")) if showMessages
      elsif showMessages
    ...
    if ChallengeModes.had_first_encounter?(target)
      if showMessages
        ...
        pbSEStop
        pbMessage(_INTL("Ya has tenido un encuentro en este mapa!", rule_name))
      end
      next false
    end
    next true
